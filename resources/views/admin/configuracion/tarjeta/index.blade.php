...
@section('content')
    ...
    <h1 class="flex items-center gap-2 text-sm font-semibold text-slate-700 mb-4">
        <i class="fa-regular fa-credit-card text-slate-600"></i>
        Listado de Tarjetas
    </h1>
                    ...
                    <tr class="border-b border-slate-100">
                        ...
                        <td class="py-2">
                            <div class="flex justify-end gap-2">
                                <button type="button"
                                    onclick="document.getElementById('modalEditarTarjeta{{ $tarjeta->id }}').classList.remove('hidden')"
                                    class="px-4 py-1.5 rounded-md text-white text-xs font-semibold"
                                    style="background-color: #64DD17;">
                                    Editar
                                </button>
                                <div id="modalEditarTarjeta{{ $tarjeta->id }}"
                                    class="hidden fixed inset-0 z-[99999] flex items-start justify-center pt-6 sm:pt-24 overflow-y-auto">
                                    <div class="absolute inset-0 bg-black/10"
                                        onclick="document.getElementById('modalEditarTarjeta{{ $tarjeta->id }}').classList.add('hidden')">
                                    </div>
                                    <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-2xl mx-3 sm:mx-4 p-4 sm:p-6 my-4 sm:my-0">
                                        ...
                                        <form action="{{ route('configuracion.tarjeta.update', $tarjeta->id) }}"
                                            method="POST">
                                            @csrf
                                            @method('PUT')
                                            <div class="flex flex-col md:flex-row gap-4">
                                                <!-- Código -->
                                                <div class="flex-1">
                                                    <label for="codigo{{ $tarjeta->id }}"class="block text-sm font-medium text-blue-700 mb-1">
                                                        Código
                                                    </label>
                                                    <input type="text" id="codigo{{ $tarjeta->id }}" name="codigo" value="{{ $tarjeta->codigo }}"
                                                ...
                                                <!-- Descripción -->
                                                <div class="flex-1">
                                                    <label for="descripcion{{ $tarjeta->id }}" class="block text-sm font-medium text-blue-700 mb-1">
                                                        Descripción
                                                    </label>
                                                    <input type="text" id="descripcion{{ $tarjeta->id }}" name="descripcion"value="{{ $tarjeta->descripcion }}"
                                                        class="w-full rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-400">
                                                </div>
                                            </div>
                                ...
                                <!-- ELIMINAR -->
                                <button type="button"
                                    onclick="confirmarEliminar('{{ route('configuracion.tarjeta.destroy', $tarjeta->id) }}')"
                                    class="px-4 py-1.5 rounded-md text-white text-xs font-semibold"
    ...
    <div id="modalNuevoTarjeta"
        class="hidden fixed inset-0 z-[99999] flex items-start justify-center pt-6 sm:pt-24 overflow-y-auto">
        <div class="absolute inset-0 bg-black/10"
            onclick="document.getElementById('modalNuevoTarjeta').classList.add('hidden')">
        </div>
        <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-4xl mx-3 sm:mx-4 p-4 sm:p-6 my-4 sm:my-0">
            ...
            <form action="{{ route('configuracion.tarjeta.store') }}" method="POST">
                @csrf
                <div class="flex flex-col md:flex-row gap-4">
                    <!-- Código -->
                    <div class="flex-1">
