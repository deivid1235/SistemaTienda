...
@section('title', 'Configuración - Atributos')
@section('js')
@endsection
@section('content')
    ...
    <div class="bg-white rounded-3xl shadow-md p-6">
        <div class="flex justify-end">
            <button type="button"
                onclick="document.getElementById('modalNuevoAtributo').classList.remove('hidden')"
        ...
        <div class="overflow-x-auto mt-4">
            <table class="w-full min-w-[700px] text-left text-sm">
                ...
                <tbody>
                    @forelse($atributos as $index => $atributo)
                        <tr class="border-b border-slate-100">
                            <td class="py-2 text-slate-600">{{ $index + 1 }}</td>
                            <td class="py-2 text-black font-medium">{{ $atributo->codigo }}</td>
                            <td class="py-2 text-black font-medium">{{ $atributo->descripcion }}</td>
                            <td class="py-3 px-4">
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold
                                    {{ $atributo->estado === 'Si' || $atributo->estado === 'Sí' 
                                        ? 'bg-emerald-50 text-emerald-700 ring-1 ring-inset ring-emerald-600/20' 
                                        : 'bg-rose-50 text-rose-700 ring-1 ring-inset ring-rose-600/20' }}">
                                    <span class="w-1.5 h-1.5 rounded-full {{ $atributo->estado === 'Si' || $atributo->estado === 'Sí' ? 'bg-emerald-500' : 'bg-rose-500' }}"></span>
                            ...
                            <td class="py-2">
                                <div class="flex justify-end gap-2">
                                    <button type="button"
                                        onclick="document.getElementById('modalEditarAtributo{{ $atributo->id }}').classList.remove('hidden')"
                                        class="px-4 py-1.5 rounded-md text-white text-xs font-semibold"
                                        style="background-color: #64DD17;">
                                        Editar
                                    </button>
                                    {{-- MODAL EDITAR --}}
                                    <div id="modalEditarAtributo{{ $atributo->id }}"
                                        class="hidden fixed inset-0 z-[99999] flex items-start justify-center pt-6 sm:pt-24 overflow-y-auto">
                                        <div class="absolute inset-0 bg-black/10"
                                            onclick="document.getElementById('modalEditarAtributo{{ $atributo->id }}').classList.add('hidden')">
                                        </div>
                                        <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-5xl mx-3 sm:mx-4 p-4 sm:p-6 my-4 sm:my-0">
                                            ...
                                            <form action="{{ route('configuracion.atributo.update', $atributo->id) }}"
                                                method="POST">
                                                @csrf
                                                @method('PUT')
                                                <div class="flex flex-col md:flex-row gap-4">
                                                    {{-- codigo --}}
                                                    <div class="w-40">
                                                        <label for="codigo_atributo{{ $atributo->id }}" class="block text-sm font-medium text-black mb-1">
                                                            Código
                                                        </label>
                                                        <input type="text" id="codigo_atributo{{ $atributo->id }}" name="codigo" value="{{ $atributo->codigo }}" maxlength="20" required
                                                    ...
                                                    {{-- Descripción --}}
                                                    <div class="flex-1">
                                                        <label for="descripcion_atributo{{ $atributo->id }}" class="block text-sm font-medium text-black mb-1">
                                                            Descripción
                                                        </label>
                                                        <input type="text" id="descripcion_atributo{{ $atributo->id }}" name="descripcion" value="{{ $atributo->descripcion }}" maxlength="150" required
                                                        class="w-full rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-400">
                                                    </div>
                                                </div>
                                    ...
                                    {{-- ELIMINAR --}}
                                    <button type="button"
                                        onclick="confirmarEliminar('{{ route('configuracion.atributo.destroy', $atributo->id) }}')"
                                        class="px-4 py-1.5 rounded-md text-white text-xs font-semibold"
    ...
    <div id="modalNuevoAtributo"
        class="hidden fixed inset-0 z-[99999] flex items-start justify-center pt-6 sm:pt-24 overflow-y-auto">
        <div class="absolute inset-0 bg-black/10"
            onclick="document.getElementById('modalNuevoAtributo').classList.add('hidden')">
        </div>
        <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-5xl mx-3 sm:mx-4 p-4 sm:p-6 my-4 sm:my-0">
            <div class="flex items-center justify-between mb-6">
                <h2 class="text-lg font-semibold text-slate-800">
            ...
            <form action="{{ route('configuracion.atributo.store') }}" method="POST">
                @csrf
                <div class="flex flex-col md:flex-row gap-4">
                    {{--codigo --}}
                    <div class="w-48">
                        <label for="codigo_atributo" class="block text-sm font-medium text-black mb-1">
                            Código
                        </label>
                        <input type="text" id="codigo_atributo" name="codigo" maxlength="20"required
                        class="w-full rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-400">
                    </div>

                    {{--Descripción --}}
                    <div class="flex-1">
                        <label for="descripcion_atributo" class="block text-sm font-medium text-black mb-1">
                            Descripción
                        </label>
                        <input type="text" id="descripcion_atributo" name="descripcion" maxlength="150" required
                        class="w-full rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-400">
                    </div>
                </div>
                {{-- Estado --}}
